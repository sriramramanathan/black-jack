...
import { MAX_PLAYERS, STARTING_CHIPS, MIN_BET, } from './gameTypes';
...
export class BlackjackGame {
    ...
    constructor({ numberOfDecks = 1, deck } = {}) {
    ...
    static fromJSON(json) {
        const { state, numberOfDecks } = JSON.parse(json);
    ...
    addPlayer(name) {
        ...
        if (this.state.phase !== 'waiting_for_players' && this.state.phase !== 'round_over') {
            throw new Error(`Cannot join during '${this.state.phase}' — new players can only join before the game starts or between rounds`);
        }
        if (!name.trim()) {
    ...
    double(playerId) {
        ...
        if (hand.bet > player.chips) {
            throw new Error(`${player.name} needs ${hand.bet} more chips to double but only has ${player.chips}`);
        }
    ...
    split(playerId) {
        ...
        if (hand.bet > player.chips) {
            throw new Error(`${player.name} needs ${hand.bet} more chips to split but only has ${player.chips}`);
        }
